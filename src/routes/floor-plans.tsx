  ...
  head: () => ({
    meta: [
      { title: "Floor Plans & Specs — Pura Vida Villas | EarthAroma" },
      {
        name: "description",
        content:
          "Interactive 3600 sq.ft. villa floor plans (G+2), site plan, and complete specification sheet — Italian marble, Duravit, Grohe, glass capsule lift.",
      },
      { property: "og:title", content: "EarthAroma Floor Plans" },
      {
        property: "og:description",
        content: "Built to the finest detail — explore every plan and specification.",
      },
    ],
  }),
  ...
  return (
    <>
      <PageHeader
        ...
        intro="Four configurations across G+2 plus a private terrace — explore every level."
      />
      <FloorPlanViewer />
      ...
      <section className="bg-earth px-6 py-24 md:px-16 text-cream">
        <div className="mx-auto max-w-6xl">
          <FadeUp>
            <div className="section-label" style={{ color: "var(--sand)" }}>
              Villa Specification
            </div>
            ...
            <p className="mt-6 max-w-2xl text-[0.92rem] leading-[1.8] text-sand-light/80">
              From Italian marble flooring and Duravit sanitary ware to concealed copper wiring and
              hydro-pneumatic plumbing — every specification reflects uncompromising quality.
            </p>
            <div className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {specPreview.map((s) => (
                <div
                  key={s.label}
                  className="flex items-start gap-3 rounded border border-sand/20 bg-white/5 p-4"
                >
                ...
                <div className="rounded bg-white p-6">
                  ...
                  <ul className="space-y-2">
                    {sec.items.map((it) => (
                      <li
                        key={it}
                        className="relative pl-5 text-[0.82rem] leading-[1.6] text-ink-mid"
                      >
