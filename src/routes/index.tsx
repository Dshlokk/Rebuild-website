...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "IGBC Platinum certified 3600 sq.ft. eco-luxury villas. Biophilic design, lakeside serenity, 100% renewable energy at EarthAroma." },
    ...
    <>
      ...
      <section className="relative bg-white px-6 py-24 md:px-16">
        <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-16 md:grid-cols-2">
          <FadeUp className="relative h-[420px] md:h-[520px]">
            <img
              src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=900&q=80"
              alt="Pura Vida villa"
              className="absolute right-0 top-0 h-[80%] w-[78%] rounded object-cover"
            />
            <img
              src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&q=80"
              alt="Tropical garden"
              className="absolute bottom-0 left-0 h-[52%] w-[52%] rounded border-[6px] border-white object-cover"
            />
          </FadeUp>
          <FadeUp delay={0.1}>
            <div className="section-label">The Philosophy</div>
            <h2 className="section-title">The Art of<br /><em>Pure Living</em></h2>
            ...
            <p className="text-[0.93rem] leading-[1.85] text-ink-mid">
              In Costa Rica, <em>Pura Vida</em> represents the relaxed, active attitude toward life. At EarthAroma,
              we've translated this ethos into architecture, community, and lifestyle. Sustainability isn't a feature.
              Nature isn't an amenity. And luxury isn't an escape — it's simply home.
            </p>
            <div className="mt-8 grid grid-cols-3 gap-3">
              {[["3600", "Sq.Ft. per Villa"], ["100%", "Renewable Energy"], ["G+2", "Floors + Terrace"]].map(([n, l]) => (
                <div key={l} className="rounded bg-cream p-4 text-center">
                  <div className="font-display text-3xl text-forest">{n}</div>
                  <div className="mt-1 text-[0.66rem] uppercase tracking-[0.14em] text-ink-muted">{l}</div>
                </div>
              ))}
            </div>
            <Link to="/about" className="btn-outline-forest mt-8">Read Our Story</Link>
          </FadeUp>
        </div>
      </section>

      {/* IGBC */}
      <section className="relative bg-forest px-6 py-24 md:px-16">
        <div className="absolute inset-x-0 top-0 h-1.5 opacity-40" style={{ background: "repeating-linear-gradient(90deg, var(--gold) 0, var(--gold) 8px, transparent 8px, transparent 24px)" }} />
        <div className="mx-auto max-w-6xl">
          <FadeUp>
            <div className="section-label" style={{ color: "var(--forest-pale)" }}>Certification</div>
            <h2 className="section-title text-cream max-w-xl">What Makes an IGBC<br /><em>Platinum Home</em> Special?</h2>
          </FadeUp>
          <Stagger>
        ...
        <div className="mx-auto max-w-6xl">
          <FadeUp className="mb-12 flex flex-wrap items-end justify-between gap-6">
            <div>
              ...
              <h2 className="section-title">#RootedIn<em>Responsibility</em></h2>
            </div>
            <Link to="/features" className="btn-outline-forest">All Features</Link>
          </FadeUp>
          <Stagger>
              ...
              {features.map((f) => (
                ...
                >
                  <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-full border border-cream-dark bg-cream">
                    <svg viewBox="0 0 24 24" className="h-5 w-5 stroke-forest" fill="none" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                      <path d={f.path} />
                    </svg>
                  </div>
