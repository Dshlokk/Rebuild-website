  ...
  head: () => ({
    meta: [
      { title: "Amenities — Pura Vida Villas | Lakeside Clubhouse & Wellness" },
      {
        name: "description",
        content:
          "Tennis, swimming, gym, lakeside walks, spa, gazebo and more — community amenities for athletes and seekers of serenity.",
      },
...
function Grid({ title, items }: { title: string; items: { name: string; icon: string }[] }) {
  return (
    <div>
      <h3 className="mb-6 border-b border-cream-dark pb-3 font-display text-2xl text-forest">
        {title}
      </h3>
      <Stagger>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {items.map((a) => (
            ...
            >
              <svg
                viewBox="0 0 24 24"
                className="h-6 w-6 stroke-forest-mid transition-colors group-hover:stroke-cream"
                fill="none"
                strokeWidth="1.5"
                strokeLinecap="round"
  ...
  return (
    <>
      <PageHeader
        ...
        title={
          <>
            #Luxury Is <em>Home</em>
          </>
        }
        intro="A lakeside clubhouse, biodynamic gardens, and spaces designed for wellness, sport, and slow evenings."
      />
